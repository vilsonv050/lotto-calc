        ...
        <aside class="workbench-right stack">
          <article class="panel archive-ready-card">
            ...
            <div class="button-row command-row archive-command-row">
              ${commandWithHelp(`<button class="secondary-btn" data-action="update-draws">Обновить с НЛОТО</button>`, "Пытается получить свежие тиражи выбранной лотереи с сайта НЛОТО и добавить их в архив.")}
              ${commandWithHelp(`<button class="ghost-btn" data-action="choose-import-file">Загрузить CSV/TXT</button>`, "Открывает файл формата: номер, дата, восемь чисел поля 1 и число поля 2.")}
...
function strategyPanel(profile) {
  ...
  const historyPageCount = Math.max(
    1,
    Math.ceil((result?.entries.length || 0) / historyPageSize),
  );
      ...
      <section class="strategy-grid">
        ...
        <article class="panel strategy-rules">
          ...
          <div class="form-grid">
            ...
            <label>${fieldTitle("Правило после проигрыша", "После проигрыша количество либо увеличивается на N, либо умножается на N.")}
            ...
            <label>${fieldTitle("Шаг N", "Число, которое используется в выбранном правиле после проигрыша.")}
            ...
            <label>${fieldTitle("Лимит затрат, ₽", "Если значение больше нуля, расчёт остановится при достижении заданного общего расхода.")}
              <input type="number" min="0" step="1" data-strategy="budget" value="${layer.budget}">
            </label>
            <label>${fieldTitle("Условие выигрыша", "Определяет, когда считать стратегический цикл выигранным: любая выплата, прибыль, возврат просадки или выбранная категория.")}
          ...
          <div class="check-row">
            <label class="check-control"><input type="checkbox" data-strategy="resetOnWin" ${layer.resetOnWin ? "checked" : ""}><span>Сбросить к начальному количеству после выигрыша</span>${helpTip("После срабатывания условия выигрыша следующий тираж снова начинается с начального количества.")}</label>
            <label class="check-control"><input type="checkbox" data-strategy="stopOnWin" ${layer.stopOnWin ? "checked" : ""}><span>Остановить ставку или слой после выигрыша</span>${helpTip("После срабатывания условия выигрыша эта ставка или весь реальный комплект больше не участвует в следующих тиражах.")}</label>
          </div>
        ...
        result
          ? `
            <article class="panel strategy-result">
              <div class="panel-heading">
                <div><span class="eyebrow">Прогон «${escapeHtml(layer.name)}» · ${escapeHtml(result.scopeLabel || strategyScope.label)}</span><h2>${result.entries.length} рассчитанных тиражей</h2></div>
              ...
              <div class="strategy-history">
                <div class="strategy-history-tools">
                  ...
                  <span>Показаны <strong>${result.entries.length ? historyStart + 1 : 0}–${historyEnd}</strong> из <strong>${result.entries.length.toLocaleString("ru-RU")}</strong></span>
                </div>
                <div class="strategy-history-head"><span>Тираж</span><span>Кол-во</span><span>Затраты</span><span>Выигрыш</span><span>Итог</span><span>Баланс ${helpTip("Перед первым тиражом в баланс вносится полная сумма купленных билетов за выбранный период. Затем после каждого тиража вычитаются его затраты и добавляется выигрыш.")}</span></div>
                  ...
                  .map(
                    (entry) => `
                      <div class="strategy-history-row">
                        <strong>№${escapeHtml(entry.drawNum)}</strong>
  ...
  return `
    <section class="stack">
      <section class="view-grid draws-tools">
        <article class="panel">
          <div class="panel-heading">
            <div><span class="eyebrow">Архив ${game.name}</span><h2>Загрузка тиражей ${helpTip("Архив выбранной лотереи можно обновить автоматически либо загрузить из CSV/TXT-файла.")}</h2></div>
          ...
          <div class="button-row command-row">
            ${commandWithHelp(`<button class="primary-btn" data-action="update-draws">Обновить с НЛОТО</button>`, "Получает доступные тиражи текущей лотереи с сайта и добавляет их без удаления ручного архива.")}
            ${commandWithHelp(`<button class="secondary-btn" data-action="choose-import-file">Импорт из файла</button>`, "Читает JSON, CSV или TXT и сначала показывает предварительную проверку строк.")}
        ...
        <div class="archive-search-content">
          ...
          ${
            profile.draws.length
              ...
              : `<div class="empty-state">Обновите архив с НЛОТО или импортируйте файл.</div>`
          }
        </div>
      </details>
    </section>`;
    ...
    <section class="stack">
      <section class="view-grid payout-meta-grid">
        <article class="panel source-version">
          <span class="eyebrow">Данные сайта</span>
          <h2>${game.name}</h2>
          <dl>
            <div><dt>Источник</dt><dd>${escapeHtml(profile.siteMeta.source)}</dd></div>
            <div><dt>Тираж</dt><dd>${escapeHtml(profile.siteMeta.drawNum)}</dd></div>
            <div><dt>Получено</dt><dd>${escapeHtml(profile.siteMeta.date)}</dd></div>
            <div><dt>Цена сайта</dt><dd>${formatMoney(profile.sitePrice)}</dd></div>
          </dl>
          ${commandWithHelp(`<button class="primary-btn wide" data-action="update-payouts">Обновить с сайта</button>`, "Получает актуальную цену и таблицу выплат НЛОТО, но не затирает вашу сохранённую ручную версию.")}
          <p class="help-text">Обновление меняет только копию данных сайта и не затирает ручную таблицу.</p>
        </article>
        <article class="panel source-version">
          <span class="eyebrow">Активная пользовательская версия</span>
          <h2>${siteDiff ? `${siteDiff} отличий от сайта` : "Совпадает с сайтом"}</h2>
          <dl>
          ...
          ${commandWithHelp(`<button class="secondary-btn wide" data-action="restore-site-payouts">Вернуть данные сайта</button>`, "Заменяет пользовательскую таблицу последней полученной копией данных сайта. Действие выполняется только по этой кнопке.")}
...
async function updateDraws() {
  const game = currentGame();
  const profile = currentProfile();
  toast("Загружаю архив НЛОТО…");
  try {
    const response = await fetch(
      `/api/nloto/history?productId=${game.productId}`,
    );
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.error || "Ошибка загрузки");
    const rows = normaliseHistoryResponse(payload);
    const existingDraws = profile.drawsAreDemo ? [] : profile.draws;
    const report = validateImportedDraws(rows, existingDraws);
    if (!report.valid.length) {
      throw new Error("Сайт вернул архив в неизвестном формате");
    }
    profile.draws = mergeDrawArchives(existingDraws, report.valid, "add");
    profile.drawsUpdated = `НЛОТО · ${new Date().toLocaleString("ru-RU")}`;
    profile.drawsAreDemo = false;
    profile.archiveSeedFixVersion = 1;
    profile.selectedDraw = profile.draws[0]?.drawNum || "";
    runtime.drawBrowser[state.gameKey] = {
    ...
    toast(
      stored
        ? `Архив обновлён: получено ${report.valid.length} тиражей`
    ...
    toast(
      error.message ||
        "НЛОТО сейчас недоступен. Используйте импорт из файла.",
      "error",
    );
  }
}
...
async function updatePayouts() {
  const game = currentGame();
  const profile = currentProfile();
  toast("Проверяю таблицу НЛОТО…");
  try {
    const response = await fetch(
      `/api/nloto/rules?productId=${game.productId}`,
    );
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.error || "Ошибка загрузки");
    const rules = normalisePayoutRules(payload);
    if (!rules) throw new Error("Сайт вернул таблицу в неизвестном формате");
    profile.sitePayouts = [...rules.payouts];
    profile.siteMeta = {
      source: "НЛОТО · данные обновлены",
      drawNum: rules.drawNum || "текущий",
      date: new Date().toLocaleString("ru-RU"),
    };
    if (rules.price > 0) profile.sitePrice = rules.price;
    clearStrategyResults();
    save();
    render();
    toast("Данные сайта обновлены. Ручная версия не изменена.", "success");
  } catch (error) {
    toast(error.message || "Не удалось обновить выплаты", "error");
  }
}
...
function handleExtendedAction(action) {
  ...
  if (action === "generate") {
    ...
    render();
  } else if (action === "restore-site-payouts") {
    profile.customPayouts = [...profile.sitePayouts];
    profile.payoutDraft = [...profile.sitePayouts];
    profile.customMeta = {
      source: "Применены данные сайта по кнопке",
      date: new Date().toLocaleString("ru-RU"),
    };
    clearStrategyResults();
    save();
    render();
    toast("Пользовательская версия заменена данными сайта", "success");
  }
}
